...
// === Descrições ===
const SOBRE_NOS = `🏍️ *Shineray São José*\n
...
client.on('message', async msg => {
  ...
  if (veioDoAnuncio) {
    ...
    await sendWithPause(msg.from, [
      `Olá ${nomeContato}! 👋 Sou o *Everton*, atendente da Shineray São José.`,
      `Seja bem-vindo(a)! 🚀`,
      MENU_TEXT
    ]);
...
async function tratarMensagem(msg) {
  try {
    ...
    if (texto.match(/^(menu|oi|olá|ola|bom dia|boa tarde|boa noite|inicio|start|help)/i)) {
      ...
      await sendWithPause(from, [
        `Olá ${nomeContato}! 👋 Sou o *Everton*, atendente virtual da Shineray São José.`,
        MENU_TEXT
      ]);
      return;
    }
